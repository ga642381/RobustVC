...
for key in metas[0].keys():
    ...
    for subdir, meta in zip(sub_dirs, metas):
        for idx, value in enumerate(meta[key]):
            merged[key][idx]["clean_audio_path"] = value["clean_audio_path"]
            merged[key][idx]["noisy_audio_path"] = value["noisy_audio_path"]
            merged[key][idx][meta["feature_name"]] = os.path.join(
                subdir, value["feature_path"]
            )
